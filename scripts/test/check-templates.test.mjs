...
//
// check-templates.mjs reads the moulds in templates/ against five standards
...
/* Everything the guard reads: the moulds, the standards (T-11), the rules and
   readers under scripts/lib, REUSE.toml (T-04). A git repo, because the guard
   discovers files with `git ls-files`. */
function scratch() {
  const dir = mkdtempSync(path.join(tmpdir(), 'templates-'));
  for (const p of ['templates', 'standards', 'scripts', 'guards', 'canon', 'protocols', 'REUSE.toml'])
...
function run(dir) {
  const r = spawnSync('node', [path.join(dir, 'scripts/check-templates.mjs')], { cwd: dir, encoding: 'utf8' });
...
check('T-02: a file in templates/ without .md is reported — every markdown tool is blind to it', (dir) => {
  renameSync(path.join(dir, 'templates/DBT-TEMPLATE.md'), path.join(dir, 'templates/DBT-TEMPLATE'));
  execFileSync('git', ['-C', dir, 'add', '-A'], { stdio: 'ignore' });
  assert.match(run(dir).log, /T-02 is not a \.md file/);
});

check('T-02: a template that names no registered series is reported', (dir) => {
  writeFileSync(path.join(dir, 'templates/ZZZ-TEMPLATE.md'), '---\nid: ""\n---\n# x\n');
  execFileSync('git', ['-C', dir, 'add', '-A'], { stdio: 'ignore' });
  assert.match(run(dir).log, /T-02 does not name a registered series.*\n\s+templates\/ZZZ-TEMPLATE\.md/);
});

check('T-01: a mould with no frontmatter is reported', (dir) => {
  edit(dir, 'templates/DBT-TEMPLATE.md', (t) => t.replace(/^---\n[\s\S]*?\n---\n/, ''));
  assert.match(run(dir).log, /T-01 no frontmatter.*\n\s+templates\/DBT-TEMPLATE\.md/);
});

check('T-01: a mould missing a ring-1 field teaches its absence', (dir) => {
  edit(dir, 'templates/DBT-TEMPLATE.md', (t) => t.replace(/^title:.*\n/m, ''));
...
check('T-03: an inline `# comment` after a value is the D-009 shape', (dir) => {
  setFM(dir, 'templates/STD-TEMPLATE.md', 'status', 'draft  # draft|active|superseded');
  assert.match(run(dir).log, /T-03 inline comment after "status"/);
});

check('T-04: a licence that is not the DESTINATION regime is reported — templates/ itself is CC0', (dir) => {
  // debt/ is CC-BY-4.0 per REUSE.toml. A mould that copied the licence of the
  // folder it LIVES in (templates/ is CC0-1.0) would be wrong for debt/ — the
  // exact defect this guard was written for, invisible to the licence guard.
  setFM(dir, 'templates/DBT-TEMPLATE.md', 'license', '"CC0-1.0"');
...
check('T-05: a type that does not belong to the destination series is reported', (dir) => {
  setFM(dir, 'templates/MIS-TEMPLATE.md', 'type', 'documentation');
...
check('T-06: a status outside the destination lifecycle is reported', (dir) => {
  setFM(dir, 'templates/MIS-TEMPLATE.md', 'status', 'closed');
...
check('T-08: a version that is not bare SemVer is reported', (dir) => {
  setFM(dir, 'templates/DBT-TEMPLATE.md', 'version', '"v0.1.0"');
...
check('T-08: a mould that opens above 0.1.0 is reported — a new artifact starts there (VER-021)', (dir) => {
  setFM(dir, 'templates/DBT-TEMPLATE.md', 'version', '"1.0.0"');
...
check('T-09: a context card without its Pragmatic line is reported', (dir) => {
  edit(dir, 'templates/DBT-TEMPLATE.md', (t) => t.replace(/^> \*\*Pragmatic:\*\*.*\n/m, ''));
...
check('the companions (MIS-TEMPLATE-EXAMPLE, -CHANGES) are records, not moulds: destination checks do not apply', (dir) => {
  ...
  setFM(dir, 'templates/MIS-TEMPLATE-EXAMPLE.md', 'version', '"3.2.1"');
  const r = run(dir);
  assert.doesNotMatch(r.log, /T-0[4-9] .*\n\s+templates\/MIS-TEMPLATE-EXAMPLE\.md/, r.log);
});
...
check('T-10: a registered series with no mould is reported by the file it lacks', (dir) => {
  execFileSync('git', ['-C', dir, 'rm', '-q', 'templates/DBT-TEMPLATE.md'], { stdio: 'ignore' });
