...
- head: `ebd1dbc`  · corpus_hash: `1137deb75a22432e…`  · measured_at: 2026-09-20T08:54:52Z  · root_dirty: 0

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 491 | files | `git ls-files` at HEAD, every path |
| `corpus.files_by_ext` | (table below) | files | tracked files by lowercase extension; `(none)` when no extension |
| `corpus.md_total` | 170 | files | tracked `.md` anywhere, including `web/` |
| `corpus.docs_total` | 168 | documents | tracked `.md` outside `web/` — the corpus every other family measures |
...
| `corpus.scripts_total` | 33 | files | files under `scripts/` with a code extension (.py .mjs .js .sh .ts) |
...
| `series.registered_total` | 92 | documents | sum of `registration[*].registered` |
| `series.registrable_total` | 92 | documents | sum of `registration[*].total` |
...
| `tokens.total` | 601033 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 7.25 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| key | value | unit | definition |
|---|---|---|---|
| `headers.docs_with_frontmatter` | 145 | documents | docs whose text opens with a --- block the shared reader parses |
...
| `provenance.dates_vs_commits_compared` | 131 | documents | docs with a created date AND a first-add commit found by one `git log --diff-filter=AR -M` walk (renames followed) |
...
| `contradictions.ci_markers_std001` | 34 | rows | table rows of STD-001 carrying `[CI]` |
...
| key | value | unit | definition |
|---|---|---|---|
| `figures.live` | 273 | lines | lines in non-apparatus docs outside telemetry/ (frontmatter and code fences excluded) that state a corpus-shaped figure — "N tokens\|documents\|files\|missions", "N/M", "N %" — with no `@ <7-hex head>` on the line. A detector, not a verdict: dated tables and closed records legitimately carry such lines |
