...
**MON-002 — People aggregated.** Staff cost MUST enter as one line per
person per month with a code, never a name, and MUST be published only
aggregated.
...
1. **Gather the documents.** Oracle or gestoría: the month's supplier
   invoices, the payroll totals per person (code, gross, employer's social
   security) and the payment processor's monthly report.
...
3. **Write the staff lines.** Agent: one line per person code, company cost
   for the month, share of hours in development if recorded.
