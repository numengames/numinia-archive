...
- head: `0ffa0c0`  · corpus_hash: `34bcf57a47e6dc50…`  · measured_at: 2026-09-24T09:14:10Z  · root_dirty: 0

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 544 | files | `git ls-files` at HEAD, every path |
...
| `tokens.total` | 620379 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 7.09 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
