...
- head: `b1ea699`  · corpus_hash: `deec3cc7f6036ed2…`  · measured_at: 2026-09-23T16:49:48Z  · root_dirty: 0

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 526 | files | `git ls-files` at HEAD, every path |
| `corpus.files_by_ext` | (table below) | files | tracked files by lowercase extension; `(none)` when no extension |
| `corpus.md_total` | 185 | files | tracked `.md` anywhere, including `web/` |
| `corpus.docs_total` | 183 | documents | tracked `.md` outside `web/` — the corpus every other family measures |
...
| `series.registered_total` | 106 | documents | sum of `registration[*].registered` |
| `series.registrable_total` | 106 | documents | sum of `registration[*].total` |
...
| `tokens.total` | 625175 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 6.98 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| key | value | unit | definition |
|---|---|---|---|
| `headers.docs_with_frontmatter` | 160 | documents | docs whose text opens with a --- block the shared reader parses |
