...
- head: `c0c43f0`  · corpus_hash: `a7f76f6fe1e80269…`  · measured_at: 2026-09-24T15:15:47Z  · root_dirty: 0

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 569 | files | `git ls-files` at HEAD, every path |
| `corpus.files_by_ext` | (table below) | files | tracked files by lowercase extension; `(none)` when no extension |
| `corpus.md_total` | 220 | files | tracked `.md` anywhere, including `web/` |
| `corpus.docs_total` | 219 | documents | tracked `.md` outside `web/` — the corpus every other family measures |
...
| `series.registered_total` | 123 | documents | sum of `registration[*].registered` |
| `series.registrable_total` | 123 | documents | sum of `registration[*].total` |
...
| `tokens.total` | 834894 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 5.48 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| `provenance.dates_vs_commits_compared` | 163 | documents | docs with a created date AND a first-add commit found by one `git log --diff-filter=AR -M` walk (renames followed) |
