...
- head: `74377e2`  · corpus_hash: `7c6609054334a226…`  · measured_at: 2026-09-23T17:18:26Z  · root_dirty: 0

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 528 | files | `git ls-files` at HEAD, every path |
| `corpus.files_by_ext` | (table below) | files | tracked files by lowercase extension; `(none)` when no extension |
| `corpus.md_total` | 187 | files | tracked `.md` anywhere, including `web/` |
| `corpus.docs_total` | 185 | documents | tracked `.md` outside `web/` — the corpus every other family measures |
...
| `corpus.apparatus` | 24 | documents | corpus documents classified apparatus by rules.json (`type: meta`, listed basename, or template path) |
| `corpus.scripts_total` | 44 | files | files under `machine/scripts/` with a code extension (.py .mjs .js .sh .ts) |
...
| `series.registered_total` | 108 | documents | sum of `registration[*].registered` |
| `series.registrable_total` | 108 | documents | sum of `registration[*].total` |
...
| `tokens.total` | 626954 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 6.96 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| `provenance.dates_vs_commits_compared` | 148 | documents | docs with a created date AND a first-add commit found by one `git log --diff-filter=AR -M` walk (renames followed) |
