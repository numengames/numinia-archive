...
- head: `b657fd8`  · corpus_hash: `2430087cd0b7ce1b…`  · measured_at: 2026-09-22T10:22:48Z  · root_dirty: 17

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 501 | files | `git ls-files` at HEAD, every path |
...
| `corpus.scripts_total` | 38 | files | files under `machine/scripts/` with a code extension (.py .mjs .js .sh .ts) |
...
| `tokens.total` | 616080 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 7.08 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
