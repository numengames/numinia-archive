...
- head: `b996f23`  · corpus_hash: `8c91dd4ed0a50a01…`  · measured_at: 2026-09-26T13:26:19Z  · root_dirty: 0

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 598 | files | `git ls-files` at HEAD, every path |
| `corpus.files_by_ext` | (table below) | files | tracked files by lowercase extension; `(none)` when no extension |
| `corpus.md_total` | 236 | files | tracked `.md` anywhere, including `web/` |
| `corpus.docs_total` | 235 | documents | tracked `.md` outside `web/` — the corpus every other family measures |
...
| `corpus.scripts_total` | 47 | files | files under `machine/scripts/` with a code extension (.py .mjs .js .sh .ts) |
...
| `series.registered_total` | 134 | documents | sum of `registration[*].registered` |
| `series.registrable_total` | 134 | documents | sum of `registration[*].total` |
...
| protocols | 15 | 15 | 0 | 0 | 100 |
| reports | 7 | 7 | 0 | 0 | 100 |
| standards | 32 | 32 | 0 | 0 | 100 |
| system | 8 | 8 | 0 | 0 | 100 |
...
| `tokens.total` | 879691 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 5.2 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| key | value | unit | definition |
|---|---|---|---|
| `headers.docs_with_frontmatter` | 188 | documents | docs whose text opens with a --- block the shared reader parses |
...
| `provenance.dates_vs_commits_compared` | 174 | documents | docs with a created date AND a first-add commit found by one `git log --diff-filter=AR -M` walk (renames followed) |
...
| `contradictions.ci_markers_std001` | 65 | rows | table rows of STD-001 carrying `[CI]` |
