...
- head: `f69de6d`  · corpus_hash: `f2498c4f2c4bacbd…`  · measured_at: 2026-09-23T17:41:00Z  · root_dirty: 0

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 529 | files | `git ls-files` at HEAD, every path |
| `corpus.files_by_ext` | (table below) | files | tracked files by lowercase extension; `(none)` when no extension |
| `corpus.md_total` | 188 | files | tracked `.md` anywhere, including `web/` |
| `corpus.docs_total` | 186 | documents | tracked `.md` outside `web/` — the corpus every other family measures |
...
| `tokens.total` | 627954 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 6.95 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| `provenance.dates_vs_commits_compared` | 149 | documents | docs with a created date AND a first-add commit found by one `git log --diff-filter=AR -M` walk (renames followed) |
