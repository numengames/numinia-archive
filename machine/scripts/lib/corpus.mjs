 ...
 * `machine/telemetry/` — so a dirty tree is reported (`root_dirty`), never hidden.
 */
...
/** All tracked paths at HEAD's index, `machine/telemetry/` excluded: the dataset does not measure itself (same exclusion as corpusHash). */
export function trackedFiles() {
  return git('ls-files').split('\n').filter((f) => f && !f.startsWith('machine/telemetry/'));
}
...
/**
 * SHA-256 over the INDEX (`git ls-files -s`: mode, blob, stage, path), machine/telemetry/
 ...
 * add machine/telemetry/, commit — the hash names the resulting tree. `--check` in CI
 * (index == HEAD) agrees.
 */
export function corpusHash() {
  const lines = git('ls-files', '-s').split('\n').filter((l) => {
    if (!l || /\tmachine\/telemetry\//.test(l)) return false;
...
export function headInfo() {
  // dirty = unstaged modifications + untracked files outside machine/telemetry/: what the index does not describe.
  const unstaged = git('diff', '--name-only', '--', '.', ':!machine/telemetry').split('\n').filter(Boolean);
  const untracked = git('ls-files', '--others', '--exclude-standard', '--', '.', ':!machine/telemetry').split('\n').filter(Boolean);
  const staged = git('diff', '--cached', '--name-only', '--', '.', ':!machine/telemetry').split('\n').filter(Boolean);
